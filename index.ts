...
import {ScimPointer} from './scim.pointer';
...
export function createPatch(input, output): Operation[] {
  ...
  return diffAny(input, output, ptr, false)
}

export function createScimPatch(input, output, rt): Operation[] {
  const ptr = new ScimPointer([]);
  ptr.rt = rt
  // a new ScimPointer gets a default path of [''] if not specified
  return diffAny(input, output, ptr, false);
}
