...
export function intersection<T>(objects: T[]): string[] {
  ...
  objects.forEach(object => {
    for (let key in object) {
      key_counts[<string>key] = (key_counts[<string>key] || 0) + 1;
    }
  })
...
bunch of deletions.
*/
export function diffArrays<T>(input: T[], output: T[], ptr: Pointer, buildPathFilter: boolean): Operation[] {
  ...
  */
  ...
  const [operations, padding] = array_operations.reduce<[Operation[], number]>(([operations, padding], array_operation) => {
    if (isArrayAdd(array_operation)) {
      ...
      let val: any = [array_operation.value]
      const operation = {
        op: array_operation.op,
        //path: ptr.add(index_token).toString(),
        // SCIM patch has no concept of index in array
        path: ptr.toString(),
        value: val,
      };
      ...
      let inObj = <any>input[array_operation.index]
      let filter: string = ''
      if(buildPathFilter) {
        filter = ptr.rt.createPathExpr(inObj)
        if(filter != '') {
          filter = '[' + filter + ']' 
        }
      }
      ...
      if (isArrayRemove(array_operation)) {
        ...
        let replace_ptr = ptr
        if (filter != '') {
          replace_ptr = ptr.add(filter);
        }
        const replace_operations = diffAny(array_operation.original, array_operation.value, replace_ptr, false)
...
export function diffAny(input: any, output: any, ptr: Pointer, buildPathFilter: boolean): Operation[] {
  const input_type = objectType(input)
  const output_type = objectType(output)
  if (input_type == 'array' && output_type == 'array') {
    return diffArrays(input, output, ptr, buildPathFilter)
  }
